...
var DEFAULT_MEDIA_CONSTRAINTS = {optional: [{RtpDataChannels: true}]};

WebRTCConnection.create = function(config, peers, signalingChannel){
	...
	return new WebRTCConnection(config.address, peers, rtcConnection, signalingChannel);
};
...
WebRTCConnection.prototype.readAnswer = function(description){
	var emitter = this.emitter,
		rtcSessionDescription = new nativeRTCSessionDescription(description);
	
	this.rtcConnection.setRemoteDescription(rtcSessionDescription, null, function(err){
...
WebRTCConnection.prototype.readIceCandidate = function(candidate){
	var emitter = this.emitter;
	this.rtcConnection.addIceCandidate(new nativeRTCIceCandidate(candidate), null, function(err){
		emitter.emit('error', err);
	});
};

WebRTCConnection.prototype.writeAnswer = function(){
	var self = this.emitter,
