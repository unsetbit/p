...
JSONProtocol.prototype.readRaw = function(message){
	if(message instanceof ArrayBuffer){
		interpreter.onArrayBuffer(message);
	} else {
		this.readProtocolMessage(message);
	}	
};

JSONProtocol.prototype.readProtocolMessage = function(protocolMessage){
	var MESSAGE_TYPE = this.MESSAGE_TYPE,
		message = JSON.parse(protocolMessage),
